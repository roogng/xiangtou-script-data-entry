# prompt.py
_TEMPLATE = """根据你已有的知识，提供「{place}」的以下乡村信息，并严格按照给定JSON格式返回。

【重要】
- 禁止使用任何工具、命令、脚本、代码执行或联网搜索；只根据你自身的知识直接输出。
- 必须始终输出JSON结构本身，不确定或不知道的字段就留空字符串或返回 []；绝不能用文字拒绝、解释、致歉或输出JSON以外的任何内容。
- 只输出JSON本身，不要包裹markdown代码块，不要任何额外文字或命令调用。
...
返回JSON结构：
{{
  "basic_info": [{{"village_intro":"村介绍","images":["http://..."]}}],
